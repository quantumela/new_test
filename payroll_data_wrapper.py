...
def render_payroll_data_management():
    ...
    try:
        # Add the new_payroll path to sys.path if not already there
        payroll_path = os.path.join(os.getcwd(), 'new_payroll')
        if payroll_path not in sys.path:
            sys.path.insert(0, payroll_path)
        
        # Also add the payroll_panels path for payroll system (note different folder name)
        ...
        try:
            ...
            # Import and run the payroll main app (note: it's app.py, not main_app.py)
            from app import *  # This will execute the payroll main app
